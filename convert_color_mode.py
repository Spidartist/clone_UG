...
for target_color_mode in ['WLI']:
    base_path = '/home/kc/manhnd/new_data/target_' + target_color_mode

    if os.path.exists(base_path):
        for sub_folder in os.listdir(base_path):
            shutil.rmtree(base_path + '/' + sub_folder)
    ...
    for dir_info in dirs:
        ...
        anatomical_position = location.split('/')[-1]
        os.mkdir(base_path + '/' + anatomical_position)

        target_imgs_name = os.listdir(location + '/' + target_color_mode)

        for color_mode in ['FICE', 'WLI', 'BLI', 'LCI']:
            os.mkdir(base_path + '/' + anatomical_position + '/' + color_mode)
            if color_mode == target_color_mode:            
                continue        

            src_imgs_name = os.listdir(location + '/' + color_mode)

            for img_name in tqdm(src_imgs_name):
                target_img_name = random.choice(target_imgs_name)

                src_img_path = location + '/' + color_mode + '/' + img_name
                tgt_img_path = location + '/' + target_color_mode + '/' + target_img_name

                new_img = convert_tgt_style(src_img_path, tgt_img_path, beta=0.1, mode='YCbCr-CbCr')
                new_img.save(base_path + '/' + anatomical_position + '/' + color_mode + '/' + img_name)
                # break
    
    print("%s: Done" % target_color_mode)
