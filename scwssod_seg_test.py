...
d = Data(metadata_file=metadata_file, test_fold=test_fold, mode='test', img_size=(480, 480), segmentation_classes=5)
...
with torch.no_grad():
    for i in tqdm(range(len(d.samples))):
        ...
        seg_out, out3, out4, out5 = net(img)
        
        mask[0] = mask[0].unsqueeze(0)
        score = dice_score(seg_out, mask[0])
